...
export function calculateCRS(input: CrsInput): CrsResult {
    ...
    const educationPoints = input.hasSpouse
        ? (EDUCATION_POINTS_WITH_SPOUSE[input.educationLevel] ?? 0)
        : (EDUCATION_POINTS_NO_SPOUSE[input.educationLevel] ?? 0);
    ...
    // Helper function to safely get language points (rounds decimals and handles missing keys)
    const getLanguagePoints = (score: number, pointsTable: Record<number, number>): number => {
        const roundedScore = Math.round(Math.min(score, 10));
        return pointsTable[roundedScore] ?? 0;
    };

    const firstLanguagePoints =
        getLanguagePoints(input.firstLanguageTest.speaking, firstLangPerSkill) +
        getLanguagePoints(input.firstLanguageTest.listening, firstLangPerSkill) +
        getLanguagePoints(input.firstLanguageTest.reading, firstLangPerSkill) +
        getLanguagePoints(input.firstLanguageTest.writing, firstLangPerSkill);
    ...
    const canadianWorkYears = Math.floor(Math.min(input.canadianWorkExperience, 5));
    const canadianWorkPoints = input.hasSpouse
        ? (CANADIAN_WORK_WITH_SPOUSE[canadianWorkYears] ?? 0)
        : (CANADIAN_WORK_NO_SPOUSE[canadianWorkYears] ?? 0);
    ...
    if (input.hasSpouse) {
        ...
        if (input.spouseEducation) {
            details.spouseEducation = SPOUSE_EDUCATION_POINTS[input.spouseEducation] ?? 0;
    ...
    // Valid Job Offer
    // NOTE: As of March 25, 2025, job offer points have been REMOVED from CRS.
    // However, a valid job offer may still be required for certain program eligibility
    // (e.g., Federal Skilled Trades, some PNP streams).
    // We keep track of this for eligibility purposes but award 0 CRS points.
    if (input.hasValidJobOffer) {
        details.jobOffer = 0; // Job offer points removed from CRS as of March 2025
        // Previously: 200 for NOC 00, 50 for NOC 0/A/B - no longer applicable
    }
    ...
    // Add score-based recommendations (updated for 2025/2026 policy)
    if (totalScore < 450) {
        recommendations.push("Your score is below the typical cutoff. Consider category-based draws (healthcare, STEM, French, trades) which may have lower requirements");
    } else if (totalScore >= 450 && totalScore < 490) {
        recommendations.push("Your score is competitive. Monitor draw cutoffs and explore Provincial Nominee Programs (PNP) which add 600 points");
    } else if (totalScore >= 490 && totalScore < 520) {
        recommendations.push("Strong score! You have good chances in Canadian Experience Class (CEC) and many category-based draws");
    } else if (totalScore >= 520) {
        recommendations.push("Excellent score! You have a high likelihood of receiving an ITA in upcoming general Express Entry draws");
    }

    // Category-based draw recommendations (2025/2026 focus areas)
    recommendations.push("TIP: IRCC now focuses on category-based draws. Strong candidates in healthcare, STEM, trades, French-language, or agriculture may receive ITAs at lower CRS scores");
    ...
    return {
        ...
        breakdown: {
            ...
            additionalPoints,
            // @ts-ignore - details is missing from return type but present in logic. Wait, interface CrsResult has details.
        },
        details,
        recommendations,
    };
