...
export default function ChatPage() {
  ...
  const displayMessages = useCallback((): HijraahChatMessage[] => {
    ...
    if (streamMessages.length > 0) {
      // Normalize content for comparison (trim strings to avoid mismatch due to whitespace)
      const dbMessageContentSet = new Set(
        dbMessages.map(m => m.content.trim())
      );

      const newStreamMessages = streamMessages.filter(
        m => !dbMessageContentSet.has(m.content.trim())
      );
